...
export const UserPageBannerSkeleton = () => {
  return <Skeleton className="w-full max-h-[200px] h-15[vh] md:h-[25vh]" />;
};

export const UserPageBanner = ({ user }: UserPageBannerProps) => {
  const { userId } = useAuth();
  const [isBannerUploadModalOpen, setIsBannerUploadModal] = useState(false);
  return (
    <div className="relative group">
      <BannerUploadModal
        userId={user.id}
        open={isBannerUploadModalOpen}
        onOpenChange={setIsBannerUploadModal}
      />
      <div
        className={cn(
          "w-full max-h-[200px] h-[15vh] md:h-[25vh] rounded-xl bg-gradient-to-r from-gray-100 to-gray-200",
      ...
      >
        {user.clerkId === userId && (
          <Button
            ...
            onClick={() => setIsBannerUploadModal(true)}
          >
